...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Shahed Hossain Limon", NULL, NULL);
    if (window == NULL)
    {
 ...
 float vertices[] = {
    ...
    -0.6f,  0.3f, 0.0f,   1.0f, 0.0f, 1.0f,
     0.6f,  0.3f, 0.0f,   1.0f, 0.0f, 1.0f,
     0.0f,  0.85f, 0.0f,   1.0f, 0.0f, 1.0f
};
    ...
    */
    ...
    glEnableVertexAttribArray(1);// enable the vertex attribute array(VAO) for a specific attribute location.
...
void processInput(GLFWwindow *window)
{
    if ((glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS) || (glfwGetKey(window, GLFW_KEY_L) == GLFW_PRESS))
